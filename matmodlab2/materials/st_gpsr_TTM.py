...
class ST_GPSR_TTM(Material):
    ...
    def eval(self, time, dtime, temp, dtemp, F0, F, stran, d, stress, X, **kwargs):
        ...
        B_matrix = np.linalg.inv(self.params["B"])
        ...
        S_ten = matrix_rep(T, 0)

        # Do an eigen_analsyis
        S_princ, Q_princ = np.linalg.eig(S_ten)
        S_princ = np.diag(S_princ)

        A_tensor = np.einsum('ik,jl', np.eye(3), B_matrix)
        fict_S = np.tensordot(A_tensor, S_princ)
        # Convert fict_S back into vector rep
        fict_T = array_rep(fict_S, (6,))

        print('T and fict T', T, fict_T)
        print('T/FT intermeidates', fict_S, S_ten)
        # check yield
        ...
        inverse_A = np.einsum('ik,jl', np.eye(3), np.linalg.inv(B_matrix))
        S_real_p = np.tensordot(inverse_A,S_ten)
        # Apply eigen rotations
        # Convert S_real back to vector
        S_real = np.dot(Q_princ, np.dot(S_real_p, np.transpose(Q_princ)))
        T_real = array_rep(S_real, (6,))
